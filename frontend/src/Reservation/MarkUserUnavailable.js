...
import dateFormat, { formatTimezone } from 'dateformat';
...
export default function MarkUserUnavailable() {
    ...
    const fetchUnavailability = () => {
        fetch(`http://127.0.0.1:5000/joineando-del-verbo-join/usersunavailability/${localStorage.getItem("userid")}`)
    ...
    const markAvailableSlot = () => {
        ...
        fetch(`http://127.0.0.1:5000/joineando-del-verbo-join/users/marktimeunavailable/${localStorage.getItem("userid")}`, request)
            .then((response) => response.json())
            .then((data) => {
                if (data !== "NO RESULT") {
                    setModalHeader("Success")
